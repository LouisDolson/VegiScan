...
const DetailScan = ({ name, image, onCloseTwo }) => {
  ...
  if (name[name.length - 1] === "n") {
    name1 = name.slice(0, -7);
    name = name.substr(-6) + " " + name.slice(0, -7);
  } else {
    name1 = name.slice(0, -6);
    name = name.substr(-5) + " " + name.slice(0, -6);
  }
  ...
  useEffect(() => {
    const fetchDescription = async () => {
      setLoading(true);
      const perintah = `Tell me about the plant ${name1} in 1 Paragraph`;
      try {
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      setLoading(true);
      const perintah = `Recommend me 5 simple recipes with ${name1} as the ingredient. Only the recipe names. split them with "<li>".`;
      try {
        const response = await fetch(
          "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=AIzaSyBZqoAxgDkv3jeLd3v_ImSmk6-3fuVmgog",
          {
  ...
  const sendPrompt = async () => {
    ...
    setLoading(true);
    const perintah = `You are only permitted to talk about ${name1}. If they talk about something else, respond to them in about two sentences saying you are not understand about what are they talking politely. Now, Listen to the following prompt and explain it as usual: ${prompt}`;
    try {
      const response = await fetch(
        "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=AIzaSyBZqoAxgDkv3jeLd3v_ImSmk6-3fuVmgog",
        {
